    ...
    <section id="thesis" className="scroll-mt-24 py-16 md:py-24 lg:py-28">
      <Container>
        ...
        <div className="mt-6 max-w-4xl">
          <h2 className="font-display text-3xl leading-tight font-light text-[var(--color-text-primary)] md:text-6xl">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {landingContent.pillars.map((pillar, index) => (
            <article
              key={pillar.title}
              className="grid h-full content-start rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-5"
            >
              ...
              <h3 className="font-display mt-3 text-2xl font-light text-[var(--color-text-primary)]">{pillar.title}</h3>
...
function SystemDiagram() {
  ...
  return (
    <div className="mt-10 overflow-hidden rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-[linear-gradient(180deg,var(--color-surface-raised),var(--color-surface))] p-5 shadow-[var(--shadow-panel)] md:mt-12 md:p-7">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.2em] text-[var(--color-accent)]">Execution chain</p>
          <h3 className="font-display mt-3 text-2xl font-light text-[var(--color-text-primary)] md:text-3xl">
            Capture -&gt; Decompose -&gt; Execute -&gt; Confirm
          </h3>
        </div>
        <div className="flex flex-wrap gap-2">
          {endpoints.map((endpoint) => (
            <span key={endpoint} className="rounded-full border border-[var(--color-border)] bg-[var(--color-surface-inset)] px-3 py-1.5 text-xs text-[var(--color-text-secondary)]">
              {endpoint}
            </span>
          ))}
        </div>
      ...
      <div className="relative mt-7 grid gap-4 lg:grid-cols-4">
        ...
        {sequence.map((step, index) => (
          <article key={step.title} className="relative rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-bg-muted)] p-5">
            ...
            <h4 className="font-display text-xl font-light text-[var(--color-text-primary)]">{step.title}</h4>
...
function PrimitiveDiagram({ index }: { index: number }) {
  ...
  return (
    <svg className="h-28 w-full rounded-[12px] border border-[var(--color-border)] bg-[var(--color-surface-inset)]" viewBox="0 0 320 112" fill="none" aria-hidden="true">
...
function MemoryBand() {
  ...
  return (
    <div className="mt-8 grid gap-6 rounded-[var(--radius-panel)] border border-[var(--color-border)] bg-[linear-gradient(180deg,var(--color-surface-raised),transparent)] p-6 md:mt-10 md:grid-cols-[0.9fr_1.1fr] md:p-9">
      <div>
        <div className="font-mono text-xs uppercase tracking-[0.2em] text-[var(--color-accent)]">Context · the difference</div>
        <h3 className="font-display mt-4 text-3xl leading-tight font-light text-[var(--color-text-primary)]">{memoryTitle}</h3>
